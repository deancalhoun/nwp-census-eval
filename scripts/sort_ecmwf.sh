...
## Forecast
FC_BASE_DIR="/glade/derecho/scratch/dcalhoun/ecmwf/ifs/fc/0.125/2t"
...
for init_time in "${INIT_TIMES[@]}"; do
    for lead_time in "${LEAD_TIMES[@]}"; do
        SOURCE_DIRS+=("$FC_BASE_DIR/$init_time/$lead_time/")
    done
done
...
## Analysis
AN_BASE_DIR="/glade/derecho/scratch/dcalhoun/ecmwf/ifs/an/0.125/2t"

cd "$AN_BASE_DIR" || exit
echo "Organizing files in $AN_BASE_DIR..."
...
echo "Files in $AN_BASE_DIR have been organized by year and month."